...
class Search():
    """
    Bayes game for simulation of emergency mission with three search areas.
    """
    ...
    def __init__(self, name):
        ...
        if self.img is None:
            print('Can\'t load file with map {}.' .format(MAP_FILE), file=sys.stderr)
            sys.exit(1)
    ...
    def draw_map(self, last_known):
        """
        Display the map of region with scale, last known position and search areas.
        """
        ...
        cv.putText(self.img, '+ = actual position', (240, 370), cv.FONT_HERSHEY_PLAIN, 1, (255, 0, 0))
    ...
    def sailor_final_location(self, num_search_areas):
        """
        Returns the x and y coordinates of the missing person's actual location.
        """
        ...
        self.sailor_actual[1] = np.random.choice(self.sa1.shape[1], 1)
    ...
    def calc_search_effectiveness(self):
        """
        Calculates a decimal value representing the search effectiveness for each area.
        """
    ...
    def conduct_search(self, area_num, area_array, effectiveness_prob):
        """
        Return the search result and a list of searched coordinates.
        """
        ...
        if area_num == self.area_actual and loc_actual in coords:
            return 'Found on area number {}.'.format(area_num), coords
        else:
            return 'Not found.', coords
        
    def revise_target_probs(self):
        """
        Updates the probability for each area based on search success.
        """
...
def draw_menu(search_num):
    """Print the menu with area to choose."""
    print('Approach no. {}'.format(search_num))
    print("""
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            print("\nIt's not correct choice.", file = sys.stderr)
            continue
        
        app.revise_target_probs()
        
        print("\nApproach number {} - result 1: {}".format(search_num, results_1), file=sys.stderr)
        print("\nApproach number {} - result 2: {}".format(search_num, results_2), file=sys.stderr)
        
        print("Effectivenes of searching (E) for approach number {}:".format(search_num))
        print("E1 = {:.3f}, E2 = {:.3f}, E3 = {:.3f}".format(app.sep1, app.sep2, app.sep3))
        
        if results_1 == 'Not found.' and results_2 == 'Not found.':
            print("\nNew probability estimation (P) for approach number {}:".format(search_num + 1))
            print("P1 = {:.3f}, P2 = {:.3f}, P3 = {:.3f}".format(app.p1, app.p2, app.p3))
        else:
